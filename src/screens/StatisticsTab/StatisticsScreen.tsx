...
export default function StatisticsScreen() {
  ...
  const [contentWidth, setContentWidth] = useState(Platform.OS === 'web' ? 0 : Dimensions.get('window').width);
  ...
  const loadStatisticsData = useCallback(async () => {
    ...
    try {
      ...
      if (monthInfo && monthInfo.startDate && monthInfo.endDate) {
        ...
        const [userProfile, userSelectedQuotas, productionEntries, supplementaryData] = await Promise.all([
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      if (Platform.OS === 'web') {
        setContentWidth(0);
        setLayoutKey(`layoutKey-${Date.now()}`);
      } else {
        setContentWidth(Dimensions.get('window').width);
      }
  ...
  const chartBlockSize = useMemo(() => {
    ...
    const calculated = (contentWidth - theme.spacing['level-4'] * 3) / 2;
    return Math.max(0, calculated);
  }, [contentWidth]);
  const smallBlockSize = useMemo(() => {
    if (contentWidth < 1) return 0;
    const calculated = (contentWidth - theme.spacing['level-4'] * 4) / 3;
    return Math.max(0, calculated);
  }, [contentWidth]);
      ...
      return (
        <View style={[styles.chartBlockBase, { width: currentChartBlockSize, height: currentChartBlockSize * 1.1 }]}>
          <PieChart
            ...
            data={[
              { value: percentage, color: ringColor, focused: true },
              { value: 100 - percentage, color: theme.colors.darkGrey },
            ]} // lightGrey -> darkGrey
            centerLabelComponent={() => (
              <View style={styles.chartCenterLabel}>
                <Text
                  style={[styles.chartValueTextMain, { fontSize: Math.max(currentChartBlockSize / 5.5, 10) }]}
                >{`${displayValue.toLocaleString()}`}</Text>
                <Text style={[styles.chartValueTextUnit, { fontSize: Math.max(currentChartBlockSize / 10, 8) }]}>
                  {unit}
                </Text>
              </View>
            )}
    ...
    (
      ...
      return (
        <View style={[styles.smallBlockBase, { width: currentSmallBlockSize, height: currentSmallBlockSize * 1.1 }]}>
          <Ionicons
            name={iconName}
            size={Math.max(currentSmallBlockSize / 3.5, 18)}
            color={iconColor}
            style={styles.smallBlockIcon}
          />
          <Text style={[styles.smallBlockValue, { fontSize: Math.max(currentSmallBlockSize / 5, 10) }]}>
            {value.toLocaleString()} <Text style={styles.smallBlockUnit}>{unit}</Text>
          </Text>
    ...
    >
      {Platform.OS === 'web' && contentWidth === 0 ? (
        ...
        <>
          ...
          <View style={[styles.rowContainer, { marginTop: theme.spacing['level-4'] }]}>
            {renderSmallInfoBlock('time-outline', totalOvertimeHours, 'Giờ tăng ca', 'giờ', theme.colors.success)}
            {renderSmallInfoBlock('walk-outline', totalLeaveDays, 'Ngày nghỉ', 'ngày', theme.colors.danger)}
            {renderSmallInfoBlock('people-outline', totalMeetingHours, 'Giờ họp/đào tạo', 'giờ', theme.colors.warning)}
          </View>
        </>
      ) : contentWidth > 0 ? (
        <View style={styles.centeredFullHeight}>
          <Text>Kích thước nội dung quá nhỏ.</Text>
...
const styles = StyleSheet.create({
  ...
  }, // md -> level-4, body.fontSize -> level-4
  errorText: {
    marginTop: theme.spacing['level-4'],
    fontSize: theme.typography.fontSize['level-4'],
  ...
  chartTitleText: {
    fontSize: theme.typography.fontSize['level-3'] * 0.9,
    fontWeight: '600',
    color: theme.colors.text,
    textAlign: 'center',
    marginTop: theme.spacing['level-1'],
  }, // bodySmall.fontSize -> level-3, xs -> level-1
  chartSubText: {
    fontSize: theme.typography.fontSize['level-2'] * 0.9,
    color: theme.colors.textSecondary,
    textAlign: 'center',
  }, // caption.fontSize -> level-2
  smallBlockBase: {
    backgroundColor: theme.colors.cardBackground,
    borderRadius: theme.borderRadius['level-7'],
  ...
  smallBlockLabel: {
    fontSize: theme.typography.fontSize['level-2'] * 0.85,
    color: theme.colors.textSecondary,
    textAlign: 'center',
    marginTop: theme.spacing['level-1'] / 2,
  }, // caption.fontSize -> level-2, xs -> level-1
  ...
  }, // bodySmall.fontSize -> level-3, sm -> level-2
